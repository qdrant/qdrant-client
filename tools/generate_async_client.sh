...
# get_collection_names (issue #1302). The transformer excludes the
# sync get_collection_names from the async mirror, and the re-injected
# async version needs to survive every regen. Fails loudly with
# non-zero exit if the close() method cannot be found (signature
# change), so a stale regen is caught immediately.
...
targets = [
    ...
    async def get_collection_names(self) -> list[str]:
        ...
        A shortcut for ``[c.name for c in (await self.get_collections()).collections]``.
        ...
        \"\"\"
        result = self._client.get_collection_names()
...
for rel, inject, needs_inspect in targets:
    ...
    if not p.exists():
        continue
    text = p.read_text()
    if "async def get_collection_names" in text:
        continue
    ...
    if close_node is None:
        print(f"regen: {rel}: close() not found; async def get_collection_names NOT re-injected (manual fix required)")
        sys.exit(1)
    lines = text.splitlines(keepends=True)
    insert_at = close_node.end_lineno
    new_text = "".join(lines[:insert_at]) + "\n" + inject + "".join(lines[insert_at:])
    p.write_text(new_text)
    print(f"regen: injected async def get_collection_names into {rel}")
PY
