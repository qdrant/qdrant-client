...
def test_upsert(collection_name: str):
    # region upload data
    points = generate_sparse_fixtures(UPLOAD_NUM_VECTORS)
    local_client = init_local()
    init_client(local_client, points, collection_name, sparse_vectors_config=sparse_vectors_config)

    remote_client = init_remote()
    init_client(
        remote_client, points, collection_name, sparse_vectors_config=sparse_vectors_config
    )
...
def test_upload_collection(collection_name: str):
    points = generate_sparse_fixtures(UPLOAD_NUM_VECTORS)
...
def test_upload_collection_generators(collection_name: str):
    points = generate_sparse_fixtures(UPLOAD_NUM_VECTORS)
...
def test_upload_points(collection_name: str):
    points = generate_sparse_fixtures(UPLOAD_NUM_VECTORS)
...
def test_upload_uuid_in_batches(collection_name: str):
    points = generate_sparse_fixtures(UPLOAD_NUM_VECTORS)
