...
def test_geo_filters_ignore_non_numeric_coordinates(qdrant: QdrantClient, geo_condition):
    ...
    valid = [{"lon": 0, "lat": 0}, {"lon": 0.0, "lat": 0.0}]
    invalid = [
    ...
    qdrant.upsert(
        "geo",
        points=[
            models.PointStruct(id=i, vector=[1.0, 0.0], payload={"location": location})
    ...
    points, _ = qdrant.scroll(
        "geo",
        scroll_filter=models.Filter(must=[models.FieldCondition(key="location", **geo_condition)]),
        limit=len(locations),
    )
    assert [point.id for point in points] == [0, 1, len(locations) - 1]
