...
def test_in_memory_selector_accepts_str_and_tuple(qdrant: QdrantClient):
    qdrant.create_collection(
        collection_name="test_collection",
        vectors_config={
            "image": models.VectorParams(size=2, distance=models.Distance.COSINE),
    ...
    qdrant.upsert(
        ...
        points=[
            models.PointStruct(
                ...
                payload={"city": "Berlin", "count": 1},
            )
        ],
    )

    # a bare string or a non-list Sequence[str] selects exactly what it names
    for selector in ("image", ["image"], ("image",)):
        record = qdrant.retrieve(
            collection_name="test_collection", ids=[1], with_vectors=selector
        )[0]
        assert record.vector is not None
        assert set(record.vector.keys()) == {"image"}, selector

    for selector in ("city", ["city"], ("city",)):
        record = qdrant.retrieve(
            collection_name="test_collection", ids=[1], with_payload=selector
        )[0]
        assert record.payload == {"city": "Berlin"}, selector

    # the same selectors apply to query results
    for selector in ("city", ("city",)):
        ...
        assert scored.payload == {"city": "Berlin"}, selector
