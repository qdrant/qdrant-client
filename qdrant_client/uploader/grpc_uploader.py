...
def upload_batch_grpc(points_client: PointsStub, collection_name: str, batch: Union[Batch, Tuple], max_retries: int) -> bool:
...
class GrpcBatchUploader(BaseUploader):
    ...
    def __init__(self, host: str, port: int, collection_name: str, max_retries: int, **kwargs: Any):
    ...
    def start(cls, collection_name: Optional[str] = None, host: str = "localhost", port: int = 6334, max_retries: int = 3, **kwargs: Any) -> 'GrpcBatchUploader':
        ...
        return cls(host=host, port=port, collection_name=collection_name, max_retries=max_retries, **kwargs)
